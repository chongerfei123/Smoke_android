...
public class ProgressBarView2 extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //绘制进度条
        progressPaint.setColor(Color.parseColor(calColor(progress / 5001, "#ff00ff00","#ffff0000")));
        ...
        textBgPaint.setColor(Color.parseColor("#7887d9"));
        ...
        canvas.drawText(text, centerX - textLen / 2 - 30, centerY - circleRadius / 10 + h1 / 2, textPaint);
        //分
        textPaint.setTextSize(20);
        textPaint.getTextBounds("ppm", 0, 1, textBounds);
        float h11 = textBounds.height();
        canvas.drawText("ppm", centerX + textLen / 2 + 8, centerY - circleRadius / 10 + h1 / 2 - (h1 - h11), textPaint);
    ...
    public void setProgress(final float progress) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 1; i <= 100; i++) {
                    float d = oldProgress + (progress - oldProgress) * (1.0f * i / 100);
                    BigDecimal bg = new BigDecimal(d).setScale(2, RoundingMode.UP);
                    ProgressBarView2.this.progress = bg.floatValue();
                    postInvalidate();
                    SystemClock.sleep(5);//设置动画的时间
                }
